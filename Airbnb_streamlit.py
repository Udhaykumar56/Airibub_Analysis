...
def datafr():
    df= pd.read_csv("C:/Users/udhay/OneDrive/Desktop/New folder/Airbnb.csv")
    return df

df= datafr()

with st.sidebar:
    select= option_menu("Main Menu", ["Home", "Data Exploration", "Contact"])
...
if select == "Data Exploration":
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("C:/Users/udhay/OneDrive/Desktop/New folder/Airbnb.csv")
            return df_a
...
if select == "Contact":
    ...
    Name = (f'{"Name :"}  {"M UDHAY KUMAR"}')
    mail = (f'{"Mail :"}  {"udhaymaddy03@gmail.com"}')
    github=(f'{"Github :"} {"https://github.com/Udhaykumar56"}')
    linkedin=(f'{"LINKEDIN :"} {"https://www.linkedin.com/in/m-udhay-kumar"}')

    st.write('''***The goal of this project is to optimize the pricing strategy for new house listings on Airbnb. By analyzing pricing data from surrounding areas, we aim to answer questions such as:***''')
    
    st.write('''1.How do other hosts price their listings in nearby locations?''')
    st.write('''2.What features (such as location,reviews, and number of beds) impact pricing decisions?''')
    st.write('''3.How can hosts set competitive prices?''')
    
    st.subheader(":orange[Scraping Data from MongoAtlas:]")

    st.write('''We collect relevant data from Airbnb listings, including details about properties, amenities, and reviews.''')
    st.write('''This step involves web scraping techniques to extract information.''')
    
    st.subheader(":orange[Dataset Cleanup and Pre-processing:]")

    st.write('''We clean and preprocess the scraped data to ensure its quality and consistency.''')
    st.write('''Handling missing values, removing outliers, and transforming features are essential steps.''')
    
    st.subheader(":orange[Data Visualization and Trend Analysis:]")

    st.write('''We create visualizations to explore patterns, correlations, and trends.''')
    st.write('''Insights gained from visualizations guide pricing decisions.''')
    st.write('''Consider using tools like Matplotlib, Seaborn, or Plotly for visualizations.''')
